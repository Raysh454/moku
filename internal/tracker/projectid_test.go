...
func TestSetAndGetProjectID(t *testing.T) {
	...
	tr, err := NewSQLiteTracker(&Config{StoragePath: siteDir}, logger)
	if err != nil {
...
func TestNewSQLiteTrackerWithConfig_ProjectID(t *testing.T) {
	...
	trA, err := NewSQLiteTracker(cfgA, logger)
	if err != nil {
	...
	_, err = NewSQLiteTracker(cfgB, logger)
	if err == nil {
	...
	trC, err := NewSQLiteTracker(cfgC, logger)
	if err != nil {
