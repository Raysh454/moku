...
func TestCheckWebSocketOrigin_AllowsWhenInAllowlist(t *testing.T) {
	t.Parallel()
	s := &Server{allowedOrigins: []string{"http://trusted.example"}}
	req := httptest.NewRequest(http.MethodGet, "/ws", nil)
	req.Header.Set("Origin", "http://trusted.example")
	if !s.checkWebSocketOrigin(req) {
		t.Error("expected allowed origin to pass WS CheckOrigin")
	}
}

func TestCheckWebSocketOrigin_RejectsWhenNotInAllowlist(t *testing.T) {
	t.Parallel()
	s := &Server{allowedOrigins: []string{"http://trusted.example"}}
	req := httptest.NewRequest(http.MethodGet, "/ws", nil)
	req.Header.Set("Origin", "http://evil.example")
	if s.checkWebSocketOrigin(req) {
		t.Error("expected rejected origin to fail WS CheckOrigin")
	}
}

func TestCheckWebSocketOrigin_AllowsWhenNoOriginHeader(t *testing.T) {
	t.Parallel()
	s := &Server{allowedOrigins: []string{"http://trusted.example"}}
	req := httptest.NewRequest(http.MethodGet, "/ws", nil)
	if !s.checkWebSocketOrigin(req) {
		t.Error("expected missing Origin header to pass (non-browser client)")
	}
}

func TestCheckWebSocketOrigin_AllowsAny_WhenAllowlistEmpty(t *testing.T) {
	t.Parallel()
	s := &Server{allowedOrigins: nil}
	req := httptest.NewRequest(http.MethodGet, "/ws", nil)
	req.Header.Set("Origin", "http://evil.example")
	if !s.checkWebSocketOrigin(req) {
		t.Error("expected empty allowlist to keep dev-permissive WS behavior")
	}
}
