...
function TreeNode({
  ...
  const changeClass = node.changeType 
    ? `treeNode-${node.changeType}` 
    : ''
  
  const icon = node.changeType === 'added' ? '+ '
  ...
  return (
    <div 
      className={`treeNodeContainer ${changeClass}`}
    ...
    >
      ...
      >
        <span className="treeNodeExpander">
          {hasChildren ? (expanded ? '▼' : '▶') : ' '}
        </span>
        <span className="treeNodeIcon">{icon}</span>
        <span className="treeNodeTag">{elementStr}</span>
        {node.textContent && (
          <span className="treeNodeText"> "{node.textContent.substring(0, 30)}..."</span>
        )}
        {node.changeDetail && (
          <span className="treeNodeChange"> ({node.changeDetail})</span>
        )}
      </div>
      {expanded && hasChildren && (
        <div className="treeNodeChildren">
          {node.children.map((child, idx) => (
            <TreeNode
              key={`${child.tagName}-${child.domIndex}-${idx}`}
              node={child}
              depth={depth + 1}
              onHover={onHover}
              onClick={onClick}
...
export default function DOMTreeView({
  ...
  if (!displayTree) {
    return <div className={`domTreeView ${className}`}>No DOM tree available</div>
  }
